...
static const char *TAG = "WIEGOTCHA";
...
{
    gpio_set_direction(d0pin, GPIO_MODE_INPUT);
    gpio_set_direction(d1pin, GPIO_MODE_INPUT);

    gpio_install_isr_service(0);
    gpio_set_intr_type(d0pin, GPIO_INTR_NEGEDGE);
    gpio_isr_handler_add(d0pin, data0Pulse, NULL);
    gpio_set_intr_type(d1pin, GPIO_INTR_NEGEDGE);
    gpio_isr_handler_add(d1pin, data1Pulse, NULL);
}
    ...
    while (1)
    {
        ...
        if (__wiegandBitCount > 0 && flagDone)
        {
            ...
            else
            {
                ...
                FILE *out2 = fopen("/littlefs/data.csv", "a");
                ESP_LOGI(TAG, "Done opening file");
                fprintf(out2, "%s,", buffer);
                fprintf(out2, "%d,", bitLen);
                fprintf(out2, "%ld,", facilityCode);
                fprintf(out2, "%ld,", cardCode);
                fprintf(out2, "%lx%06lx,", cardChunk1, cardChunk2);
                fprintf(out2, "%010lx%06lx,", maskMagicChunk1(bitLen, cardChunk1), cardChunk2);
                for (i = 19; i >= 0; i--)
                {
                    fprintf(out2, "%ld", bitRead(cardChunk1, i));
                }
                for (i = 23; i >= 0; i--)
                {
                    fprintf(out2, "%ld", bitRead(cardChunk2, i));
                }
                fprintf(out2, "\n");
                fclose(out2);
                ESP_LOGI(TAG, "%s UTC - ", buffer);
                ESP_LOGI(TAG, "Bit#:%d  ", bitLen);
                ESP_LOGI(TAG, "FC:%ld  ", facilityCode);
                ESP_LOGI(TAG, "ID:%ld  ", cardCode);
                ESP_LOGI(TAG, "Hex:%lx%06lx  ", cardChunk1, cardChunk2);
                ESP_LOGI(TAG, "Blk7:%010lx%06lx  ", maskMagicChunk1(bitLen, cardChunk1), cardChunk2);
                ESP_LOGI(TAG, "Bits: ");
                for (i = 19; i >= 0; i--)
                {
                    ESP_LOGI(TAG, "%ld", bitRead(cardChunk1, i));
                }
                for (i = 23; i >= 0; i--)
                {
                    ESP_LOGI(TAG, "%ld", bitRead(cardChunk2, i));
                }
                ESP_LOGI(TAG, "\n");
                facilityCode = 0;
                cardCode = 0;
